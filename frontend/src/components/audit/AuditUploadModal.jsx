...
export const AuditUploadModal = ({
	...
	const [fileDesc, setFileDesc] = useState(`${data.category} Q${data.index}-${files?.length + 1}`);
	...
	const onFormSubmit = async (e) => {
		...
		if (survey?.id)
			fileName = `S${survey.id}-${getValidUsername(user.name)}-${sanitizeString(
				survey?.hotelName
			)}-${sanitizeString(fileDesc)}-${Date.now()}`;
	...
	const AddFileToList = (fileData) => {
		...
		setFileDesc(`${data.category} Q${data.index}-${files?.length + 1}`);
	};
