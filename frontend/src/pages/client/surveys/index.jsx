...
const SurveyTable = dynamic(() => import('@/components/survey/SurveyTableInternal'), {
	ssr: false,
	loading: Loader,
});
...
const InternalSurveys = () => {
	...
	const fetchData = async () => {
		try {
			const res = await http.get('/survey?surveyType=INTERNAL');
			if (res?.status == 200) {
	...
	const searchRecord = async (keyword) => {
		...
		try {
			...
			if (keyword.length > 2) {
				const res = await http.get(`/survey?search=${keyword}&surveyType=INTERNAL`);
				if (res?.status == 200) {
					setData(res.data.result);
				}
			}
		...
		<Layout>
			<div className='content p-6'>
				<div className='mb-7 flex justify-between items-center'>
					...
					<Dropdown
						renderTrigger={() => (
							<div className='btn_primary _flex cursor-pointer'>
								<RiAddFill className='mr-2 h-5 w-5' />
								<span className='hidden md:block'>Create New</span>
							</div>
						)}>
						<Dropdown.Item>
							<Link href='/client/surveys/create'>Manual</Link>
						</Dropdown.Item>
						<Dropdown.Item>
							<Link href='/client/surveys/create?importFromExcel=true'>Import from Excel</Link>
						</Dropdown.Item>
					</Dropdown>
				</div>
								...
								{audits.map((audit, index) => (
									<div key={index} className='card mb-4 p-3 bg-gray-50 border border-gray-200 rounded-lg '>
										...
										<Link
											...
											target='_blank'
											className='btn_primary px-4 ml-2'>
											Print
										</Link>
									</div>
								))}
...
export default InternalSurveys;
